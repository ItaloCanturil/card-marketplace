<script lang="ts" setup>
  ...
  const handleSubmit = async () => {
    if (!offeringCard.value || !receivingCard.value) {
      return;
    }
    await useMarketplace.createTrade({
      cards: [
        {
          cardId: offeringCard.value.id,
          type: 'OFFERING'
        },
        {
          cardId: receivingCard.value.id,
          type: 'RECEIVING'
        }
      ]
    });
  ...
  <dialog id="trade_modal" class="modal" :open="showModal" @close="emit('close')">
    <div class="modal-box">
      ...
      <div class="modal-action mt-6">
        <button class="btn btn-primary w-full" :disabled="!offeringCard || !receivingCard" @click="handleSubmit">
          Enviar Proposta
        </button>
      </div>
    </div>
